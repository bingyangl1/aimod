...
/**
 * Step up: move horizontally 1 block and up 1 block simultaneously.
 * Example: src=(0,64,0) to dest=(1,65,0).
 * Differs from MovementAscend which requires diagonal (dx=1, dz=1).
 */
public class MovementStepUp extends BotMovement {

    private static final double REACH_DIST_SQR = 0.3;
    private int stuckTicks;
    private double lastDistSqr;

    public MovementStepUp(BlockPos src, BlockPos dest) {
        super(src, dest);
    }
    ...
    public double calculateCost(ServerLevel level) {
        ...
        return MoveCost.WALK_ONE_BLOCK + MoveCost.JUMP_ONE_BLOCK;
    }
    ...
    public boolean update(FakePlayer bot) {
        ...
        // Jump when close horizontally and need to go up
