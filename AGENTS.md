...
| **异步寻路** | **ai/pathing/AsyncPathfinder.java** | **后台线程寻路，不阻塞服务器 tick** |
...
- 子类：MovementTraverse（平地+搭桥）、MovementPillar（搭柱向上）
...
- [ ] Movement 子类扩展（MovementDiagonal、MovementFall）
- [ ] 路径缓存：复用最近计算的路径
