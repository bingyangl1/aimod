...
- **每次编译必须更新 `gradle.properties` 中的 `mod_version`**，递增修订号（如 `1.0.0` → `1.0.1` → `1.0.2`）。
- **每次编译必须在 `build.gradle` 的 `jar.manifest` 中写入构建时间和修复摘要**，以便区分版本和修复内容。
- 构建产物为 `build/libs/aimod-<version>.jar`，复制到 Minecraft 的 `mods/` 目录后需重启游戏生效。

### 持久约定（每次变更必须遵守）

| # | 约定 | 操作 |
|---|------|------|
| 1 | **版本号递增** | 每次 build jar：① 递增 `gradle.properties` 中 `mod_version`；② 更新 `mod_description` 描述本次修复内容；③ 更新 `build_info.json`（版本号、时间、修复摘要） |
| 2 | **文档 + Commit** | 每次变更：① 更新 `COMPREHENSIVE_ANALYSIS.md` 附录 C 变更日志；② commit message 说明修复了什么 |
| 3 | **旧 jar 清理** | `build/libs/` 只保留最新版本 jar，构建前删除旧版本 |
